...
L.Path = L.Class.extend({
	...
	redraw: function () {
		if (this._map) {
			if (typeof L.Path.CANVAS != 'undefined' && L.Path.CANVAS==true) {
				this._map._updateCanvasViewport();
			}
			this.projectLatlngs();
			this._updatePath();
		}
...
/*
 * L.Sector is a sector overlay (with a angle in degrees).
 */

L.Sector = L.Path.extend({
	initialize: function (latlng1, latlng2, angle, options) {
		L.Path.prototype.initialize.call(this, options);

		this._latlng1 = latlng1;
		this._latlng2 = latlng2;
		this._d_angle = angle;
	},
	...
	setLatLng: function (latlng1, latlng2) {
		this._latlng1 = latlng1;
		this._latlng2 = latlng2;
		return this.redraw();
	},
	...
	_calcSector: function () {
		...
		if (0 === d_x) {
			...
			if (d_y > 0) {
				r_angle = -Math.acos(d_x / r);
			} else {
				r_angle = Math.acos(d_x / r);
			}
		}

		this._r_angle = r_angle;

		this._min_angle = r_angle - (angle / 2);
		this._max_angle = r_angle + (angle / 2);


		var x3 = p1.x + r * Math.cos(this._min_angle);
		var y3 = p1.y - r * Math.sin(this._min_angle);
		this._point3 = new L.Point(x3, y3);


		var x4 = p1.x + r * Math.cos(this._max_angle);
		var y4 = p1.y - r * Math.sin(this._max_angle);
		this._point4 = new L.Point(x4, y4);

		this._big_arc = this._angle >= Math.PI ? 1 : 0;
//		this._swipe = (this._r_angle >= Math.PI && this._r_angle <= 2*Math.PI) || (this._r_angle <= -Math.PI && this._r_angle >= -2*Math.PI) ? 1 : 0;
		this._swipe = 0;

	},

	projectLatlngs: function () {
		this._point1 = this._map.latLngToLayerPoint(this._latlng1);
		this._point2 = this._map.latLngToLayerPoint(this._latlng2);
		this._radius = Math.round(Math.sqrt(Math.pow(this._point2.x - this._point1.x, 2) + Math.pow(this._point2.y - this._point1.y, 2)));
		this._angle = L.LatLng.DEG_TO_RAD * this._d_angle;
		this._calcSector();
	},

	getBounds: function () {
		var map = this._map,
			p1 = map.latLngToLayerPoint(this._latlng1),
			p2 = map.latLngToLayerPoint(this._latlng2);

		if (typeof this._point3 === 'undefined') {
			this._calcSector();
		}
		var p3 = this._point3,
			p4 = this._point4;

		var swPoint = new L.Point(Math.min(p1.x, p2.x, p3.x, p4.x), Math.min(p1.y, p2.y, p3.y, p4.y)),
		nePoint = new L.Point(Math.max(p1.x, p2.x, p3.x, p4.x), Math.max(p1.y, p2.y, p3.y, p4.y)),
		zoom = map.getZoom(),
		sw = map.layerPointToLatLng(swPoint),
		ne = map.layerPointToLatLng(nePoint);
	...
	getPathString: function () {
		...
		if (L.Browser.svg) {
			return 'M ' + p1.x + "," + p1.y + ' L ' + p2.x + ',' + p2.y + ' M ' + p1.x + "," + p1.y + ' L ' + p3.x + ',' + p3.y + ' A ' + r + ',' + r + ', 0,' + this._big_arc + ',' + this._swipe + ', ' + p4.x + ',' + p4.y + ' z';
		} else {
			p1._round();
			p2._round();
			p3._round();
			...
			var s_angle = Math.round((L.LatLng.RAD_TO_DEG * this._min_angle) * 65535);
			var to_angle = Math.round((L.LatLng.RAD_TO_DEG * this._angle) * 65535);

			return  'M ' + p1.x + " " + p1.y + ' L ' + p2.x + ' ' + p2.y + ' M ' + p1.x + " " + p1.y + ' L ' + p3.x + ' ' + p3.y + ' M ' + p1.x + " " + p1.y + ' L ' + p4.x + ' ' + p4.y + 'M ' + p1.x + " " + p1.y + ' AE '	+	p1.x	+	' '	+	p1.y	+	' '	+	r	+	' '	+	r	+	' ' + s_angle + ' ' + to_angle + ' X E';
		}
	},
	...
	_checkIfEmpty: function () {
		...
		return Math.min(p1.x, p2.x, p3.x, p4.x) > vp.max.x || Math.min(p1.y, p2.y, p3.y, p4.y) > vp.max.y ||
			Math.max(p1.x, p2.x, p3.x, p4.x) < vp.min.x || Math.max(p1.y, p2.y, p3.y, p4.y) < vp.min.y;
	}
});
...
/*
 * Sector canvas specific drawing parts.
 */
...
L.Sector.include(!L.Path.CANVAS ? {} : {
	_drawPath: function () {
		...
		this._ctx.moveTo(p1.x, p1.y);
		this._ctx.arc(p1.x, p1.y, this._radius, 2 * Math.PI - this._min_angle, 2 * Math.PI - this._max_angle, true);
	...
	_containsPoint: function (p) {
		...
		if (d_x === 0) {
			if (d_y >= 0) { r_angle = Math.PI; } else { r_angle = 0; }
		} else {
			if (d_x > 0) {
				r_angle = Math.atan(d_y / d_x) + Math.PI * 0.5;
			} else if (d_y >= 0) {
				r_angle = Math.atan(d_y / d_x) + Math.PI * 1.5;
			} else {
				r_angle = Math.atan(d_y / d_x) - Math.PI * 0.5;
			}
		}

		r_angle = r_angle - Math.PI / 2;
		return (p.distanceTo(c) <= this._radius + w2) && r_angle >= this._min_angle && r_angle <= this._max_angle;
	}
});
